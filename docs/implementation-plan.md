...
- HoneyBook remains authoritative for projects, pipeline stages, contracts, smart files, invoices, and payments. The command center mirrors the exact HoneyBook stages and owns reporting, follow-ups, mapping, attribution, health, and audit history. Zapier events and manual HoneyBook CSV snapshots update the same stable-ID records.
...
1. **Foundation — implemented:** identity/RLS, HoneyBook-first pipeline, retired Sheet archive, manual HoneyBook CSV snapshots, manual follow-ups/services, provenance, audit/sync health, accessible responsive shell.
...
- Choose automatic Zapier sync or recurring manual HoneyBook CSV snapshots. If using Zapier, create only supported triggers and configure a high-entropy shared secret.
