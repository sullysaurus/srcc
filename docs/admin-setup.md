...
Open **Integrations → HoneyBook** and create the new Zaps in a separate folder with a temporary filter for the designated test team member. Do not pause or edit the existing workflows. Create Zaps only for supported triggers: new inquiry, client created, project stage changed, project booked, payment received, and meeting scheduled. POST to `/api/webhooks/honeybook` with `x-webhook-secret`, `x-organization-id`, a unique event ID, and UTC occurrence time. Always map the stable HoneyBook project ID and exact HoneyBook stage. A new-inquiry payload must include project name, contact email, inquiry timestamp, and stage. A booking payload must include booked value. A payment payload must include payment ID plus amount. Map owner, service, event date, location, recent activity, project URL, and the website attribution token whenever the trigger exposes them. The endpoint verifies the secret, enforces the enabled state and replay window, validates the payload, deduplicates it, and retains only a redacted payload summary. The setup screen does not mark an empty test ping as ready.

Keep the parallel filter in place until at least five controlled projects have successfully exercised inquiry, stage change, proposal viewed, booking, and payment paths. Compare the resulting HoneyBook record, dashboard project, activity history, communication timestamp, and money totals before requesting a production cutover.
