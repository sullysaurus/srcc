...
export class SearchConsoleRestProvider implements SearchConsoleProvider {
  ...
  async getDaily({startDate,endDate}:{startDate:string;endDate:string}):Promise<SearchConsoleRow[]> {
    ...
    const query=async(dimensions:string[])=>{
      const response=await providerFetch(endpoint,{method:"POST",headers:{authorization:`Bearer ${this.config.accessToken}`,"content-type":"application/json"},body:JSON.stringify({startDate,endDate,dimensions,rowLimit:25000,dataState:"final"})},"search_console");
    ...
    const[detailed,appearances]=await Promise.all([
      query(["date","query","page","country","device"]),
      query(["date","searchAppearance"]),
    ]);
    return [
      ...
      ...appearances.map(row=>({date:row.keys[0]??"",query:"",page:"",country:"",device:"",searchAppearance:row.keys[1]??"",clicks:row.clicks,impressions:row.impressions,ctr:row.ctr,averagePosition:row.position})),
    ];
  }
